...
document.addEventListener('DOMContentLoaded', function() {
    ...
    request.open('GET', url_base+'assets/js/scripts/ubigeo-peru.min.json', true);
    /*  request.open('GET', 'http://angelfqc.github.io/ubigeo-peru/ubigeo-peru.min.json', true);    */
...
function showRegionsList() {
    ubigeoPeru.ubigeos.forEach(function(ubigeo) {
        if (ubigeo.provincia === '00' && ubigeo.distrito === '00') {
            ...
            label.textContent = ubigeo.nombre;
...
function showDistrictsList(departamento, provincia) {
    ubigeoPeru.ubigeos.forEach(function(ubigeo) {
        if (ubigeo.departamento === departamento && ubigeo.provincia === provincia && ubigeo.distrito !== 0) {
            ...
            input.name = 'distrito';
            input.type = 'radio';
            //input.value = ubigeo.departamento+ubigeo.provincia+ubigeo.distrito;
            input.value = ubigeo.nombre;
            var label = document.createElement('label');
