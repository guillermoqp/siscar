        ...
        <table class="table table-striped table-bordered table-hover datatable-buttons" >
            ...
            <tbody>
                <?php foreach ($seguimientos as $seguimiento) { ?>
                    <tr class="gradeX">
                        <td><?php print($seguimiento["id_seguimiento"]) ?></td>
                        <td><?php print($seguimiento["fecha_corte"]) ?></td>
                        <td><?php print($seguimiento["nro_dias_corte"]) ?></td>
                        <td><?php print($seguimiento["flag_bloqueo"]==1?"SI":"NO") ?></td>
                        <td><?php print(date("d/m/Y",strtotime($seguimiento["fecha"]))) ?></td>
                        <td><?php print($seguimiento["descripcion"]) ?></td>
                        <td><?php print($seguimiento["estado"]==1?"Activo":"Desactivo") ?></td>
                        <td>
                            <div class="dt-buttons btn-group">
                                <?php if ($this->administrarpermisos->verificarPermiso($this->session->userdata('permiso'), 'vSeguimientoDemanda')) { ?>
                                    <a href="<?php print(base_url("reportes/seguimiento_demanda_ver/".$seguimiento["id_seguimiento"])) ?>" class="btn btn-default btn-xs btn-flat" data-toggle="tooltip" data-placement="bottom" title="Ver Detalle de Seguimiento de Demanda">
                                        <i class="fa fa-eye"></i>
                                    </a>
                                <?php } ?>
                            </div>
