...
<script>
$("#roles").jqGrid({
    url: "<?php echo base_url() . 'mantenedores/cargar_roles' ?>",
    mtype: "post",
    datatype: "json",
    ...
    ondblClickRow: function (id) {
        $("#roles").editGridRow(id, {closeAfterEdit: true, mtype: 'POST'});
    },
    sortorder: "desc",
    editurl: "<?php echo base_url() . 'mantenedores/crud_roles' ?>", //URL Process CRUD
    caption: "Roles",
    ajaxRowOptions: {
        success: function () {
    ...
    gridComplete: function () {
        var ids = jQuery("#roles").jqGrid('getDataIDs');
        for (var i = 0; i < ids.length; i++) {
            var cl = ids[i];
            var data = $("#roles").jqGrid().getRowData(cl);
            if (data.IsBundled == "true") {
                be = "<input type='button' value='Ver Detalles' onclick='BundleManage(" + data.id + ",\"" + data.nombre + "\")'  />";
                jQuery("#roles").jqGrid('setRowData', ids[i], { act: be });
            }
            else {
...
ChangejQGridDesign("#roles", "#pager");
</script>