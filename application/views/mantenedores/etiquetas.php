        ...
        <div class="col-lg-12">
            <div class="ibox ">
                <div class="ibox-title">
                    <h5>Mantenedor de Etiquetas/Procesos</h5>
                </div>
                <div class="ibox-content">
                    <div class="jqGrid_wrapper">
                        <table id="etiquetas"></table>
                        <div id="pager"></div>
                    </div>
                </div>
            </div>
...
<script>
$("#etiquetas").jqGrid({
    url: "<?php echo base_url() . 'mantenedores/cargar_etiquetas' ?>",
    mtype: "post",
    datatype: "json",
    ...
    ondblClickRow: function (id) {
        $("#etiquetas").editGridRow(id, {closeAfterEdit: true, mtype: 'POST'});
    },
    sortorder: "desc",
    editurl: "<?php echo base_url() . 'mantenedores/crud_etiquetas' ?>", //URL Process CRUD
    caption: "Etiquetas/Procesos",
    ajaxRowOptions: {
        success: function () {
            $("#etiquetas").trigger("reloadGrid");
        }
    },
    gridComplete: function () {
        var ids = jQuery("#etiquetas").jqGrid('getDataIDs');
        for (var i = 0; i < ids.length; i++) {
            var cl = ids[i];
            var data = $("#etiquetas").jqGrid().getRowData(cl);
            if (data.IsBundled == "true") {
                be = "<input type='button' value='Ver Detalles' onclick='BundleManage(" + data.id + ",\"" + data.nombre + "\")'  />";
                jQuery("#etiquetas").jqGrid('setRowData', ids[i], { act: be });
            }
            else {
...
ChangejQGridDesign("#etiquetas", "#pager");
</script>